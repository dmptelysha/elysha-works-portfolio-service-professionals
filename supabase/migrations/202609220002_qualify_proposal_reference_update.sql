-- Forward-only correction for the trusted proposal finalization RPC.
-- The OUT parameter named proposal_reference shadows the quiz_sessions column
-- in PL/pgSQL, so the existing row value must be table-qualified.
